...
if __name__ == "__main__":
    ...
    if opt.model == "pranet" or opt.model == "p":
        ...
        if opt.tuning_calcification:
            ...
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/PraNet/石灰化なし含む/PraNet-best.pth"
                )
            )
        print("model:pranet")
        ...
        if opt.tuning_calcification:
            ...
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/U-PraNet/石灰化なし含む/PraNet-best.pth"
                )
            )
        ...
        if opt.tuning_calcification:
            ...
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/CaraNet/石灰化なし含む/PraNet-best.pth"
                )
            )
        print("model:caranet")
