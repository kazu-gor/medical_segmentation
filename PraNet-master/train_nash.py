...
def train(dataloaders_dict, models, optimizer, criterion, epoch, best_loss, best2_loss):
    ...
    for phase in ["train", "val"]:
        ...
        if phase == "train":
            ...
            if val_loss < best_loss:
                ...
                torch.save(
                    models["Segmentation"].state_dict(), save_path + "PraNet-best.pth"
                )
                torch.save(
            ...
            if val_d_loss < best2_loss:
                ...
                os.makedirs(save_path, exist_ok=True)
                torch.save(
                    models["Segmentation"].state_dict(), save_path + "PraNet-best2.pth"
                )
                torch.save(
                    models["Discriminator"].state_dict(),
    ...
    if (epoch + 1) % 5 == 0:
        torch.save(
            models["Segmentation"].state_dict(), save_path + "PraNet-%d.pth" % epoch
        )
        torch.save(
            models["Discriminator"].state_dict(),
...
if __name__ == "__main__":
    ...
    if opt.model == "pranet" or opt.model == "p":
        ...
        if opt.tuning:
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/PraNet/石灰化ありのみ/PraNet-best.pth"
                )
            )
        ...
        if opt.tuning:
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/U-PraNet/石灰化ありのみ/PraNet-best.pth"
                )
            )
        ...
        if opt.tuning:
            model1.load_state_dict(
                torch.load(
                    "./weights/修論/segmentation/CaraNet/石灰化ありのみ/PraNet-best.pth"
                )
            )
    ...
    if (
        opt.model == "pranet"
        or opt.model == "p"
        or opt.model == "caranet"
        or opt.model == "c"
    ):
        shared_parameters = [
            p for n, p in model1.named_parameters() if "resnet.fc" not in n
        ]
        task_specific_parameters = [
            p for n, p in model1.named_parameters() if "resnet.fc" in n
        ]
    elif opt.model == "u_pranet" or opt.model == "u":
        shared_parameters = [
            p
            for n, p in model1.named_parameters()
            if "resnet.fc" not in n
            and "vgg.classifier" not in n
            and "Decoder2" not in n
        ]
        task_specific_parameters = [
            p
            for n, p in model1.named_parameters()
            if "resnet.fc" in n or "vgg.classifier" in n or "Decoder2" in n
        ]
