...
def train(dataloaders_dict, models, optimizer, criterion, epoch, best_loss, best2_loss):
    ...
    for phase in ['train', 'val']:
        ...
        for i, pack in enumerate(dataloaders_dict[phase], start=1):
            for rate in size_rates:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    # ---- forward ----
                    lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2 = models['Segmentation'](images)
        ...
        if phase == 'train':
            ...
            if val_loss < best_loss:
                ...
                torch.save(models['Segmentation'].state_dict(), save_path + 'PraNet-best.pth')
            ...
            if val_d_loss < best2_loss:
                ...
                save_path = 'snapshots/{}/'.format(opt.train_save)
                os.makedirs(save_path, exist_ok=True)
                torch.save(models['Segmentation'].state_dict(), save_path + 'PraNet-best2.pth')
    ...
    os.makedirs(save_path, exist_ok=True)
    if (epoch + 1) % 5 == 0:
        torch.save(models['Segmentation'].state_dict(), save_path + 'PraNet-%d.pth' % epoch)
...
if __name__ == '__main__':
    ...
    if opt.model == 'pranet' or opt.model == 'p':
        ...
        if opt.tuning:
            model1.load_state_dict(
                torch.load('./weights/修論/segmentation/PraNet/石灰化ありのみ/PraNet-best.pth'))
    elif opt.model == 'u_pranet' or opt.model == 'u':
        model1 = u_pranet()
        print('model:u_pranet')
        if opt.tuning:
            model1.load_state_dict(
                torch.load('./weights/修論/segmentation/U-PraNet/石灰化ありのみ/PraNet-best.pth'))
    elif opt.model == 'caranet' or opt.model == 'c':
        model1 = caranet()
        print('model:caranet')
        if opt.tuning:
            model1.load_state_dict(
                torch.load('./weights/修論/segmentation/CaraNet/石灰化ありのみ/PraNet-best.pth'))
    ...
    if opt.model == 'pranet' or opt.model == 'p'or opt.model == 'caranet' or opt.model == 'c':
        shared_parameters = [p for n, p in model1.named_parameters() if 'resnet.fc' not in n]
        task_specific_parameters = [p for n, p in model1.named_parameters() if 'resnet.fc' in n]
    elif opt.model == 'u_pranet' or opt.model == 'u':
        shared_parameters = [p for n, p in model1.named_parameters() if 'resnet.fc' not in n
                             and 'vgg.classifier' not in n and 'Decoder2' not in n]
        task_specific_parameters = [p for n, p in model1.named_parameters() if 'resnet.fc' in n
                                    or 'vgg.classifier' in n or 'Decoder2' in n]
