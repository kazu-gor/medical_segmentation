...
def cropping_image(images, gts, outputs, opt):
    # scores = torch.tensor(
    #     outputs['pred_logits'].clone().detach()).softmax(-1)[..., :-1].max(-1)[0]
    # keep = torch.max(scores, dim=1)[0] > 0.5

    # pred_bbox = box_cxcywh_to_xyxy(torch.tensor(
    #     outputs['pred_boxes'].clone().detach())) * opt.trainsize
    pred_bbox = box_cxcywh_to_xyxy(torch.tensor(
        outputs['pred_boxes'].clone().detach())) * opt.trainsize
    print(pred_bbox)
    # pred_bbox = pred_bbox[keep].to(
    #     torch.int64).clamp(min=0, max=opt.trainsize)[0]
    pred_bbox = pred_bbox.to(
        torch.int64).clamp(
            min=0, max=opt.trainsize).reshape(4, 4)
    print(pred_bbox)
    print(pred_bbox.size())

    print(images.size())
    images = F.grid_sample(images, pred_bbox)
    gts = F.grid_sample(gts, pred_bbox)
    ...
    print(images.size())
    images = F.interpolate(images, size=(352, 352))
    gts = F.interpolate(gts, size=(352, 352))
    return images, gts
...
def train(dataloaders_dict, models, optimizer, disc_criterion, detr_criterion,
    ...
    for phase in ['train', 'val']:
        ...
        size_rates = [1]
        loss_record2, loss_record3, loss_record4, loss_record5, d_loss_record = AvgMeter(
        ...
        for i, pack in enumerate(dataloaders_dict[phase], start=1):
            for rate in size_rates:
                ...
                anno['boxes'] = anno['boxes'].reshape(4, 4)  # [4, 1, 4] to [4, 4]
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    images, gts = cropping_image(images, gts, outputs, opt)
                    ########################################

                    # ---- forward ----
                    lateral_map_5, lateral_map_4, lateral_map_3, lateral_map_2 = models['Transfuse'](
                        images)
...
if __name__ == '__main__':
    ...
    shared_parameters = [p for n, p in model1.named_parameters(
