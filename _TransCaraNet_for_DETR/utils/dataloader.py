...
class PolypDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #############################
            annotations["image_id"] = torch.tensor(index, dtype=torch.int64)
            height, width, _ = image.shape
            annotations["orig_size"] = torch.tensor([height, width])
            annotations['size'] = torch.tensor([image.shape[1], image.shape[2]])

            anno_list = self.transform_anno(gt, height, width)[0]
            # If only one bounding-box exists
            if np.array(anno_list).ndim == 1:
            ...
            if anno_list.size:
                ...
                x = anno_list[:, 0].copy()
                y = anno_list[:, 1].copy()
                xmax = anno_list[:, 2].copy()
                ymax = anno_list[:, 3].copy()

                w = (xmax - x).astype(np.float32)
                h = (ymax - y).astype(np.float32)
                area = torch.from_numpy(w * h)
                ...
                # 色チャネル：BGR to RGB
                # (高さ、幅、色チャネル) to （色チャネル、高さ、幅）
                boxes = np.hstack((boxes, np.expand_dims(anno_list[:, 4], axis=1)))[:, :4]
                annotations['boxes'] = torch.tensor(boxes, dtype=torch.float32)
                annotations['labels'] = torch.ones(len(boxes), dtype=torch.int64)
                annotations['iscrowd'] = torch.zeros(len(boxes), dtype=torch.int64)
            else:
                annotations["area"] = torch.tensor([0], dtype=torch.float32)
                annotations['boxes'] = torch.tensor([[0, 0, 0, 0]], dtype=torch.float32)
