            ...
            for rate in size_rates:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        # d_loss.backward()
                        # clip_gradient(optimizer, args.clip)
                        # clip_gradient(d_optimizer, args.clip)
                        # optimizer.step()
                        # d_optimizer.step()
...
if __name__ == '__main__':
    ...
    task_specific_parameters = [p for n, p in model1.named_parameters(
    ...
    # d_optimizer = torch.optim.Adam(params2, args.lr)
    ...
    attn_map_root_1 = Path(
        f'./dataset_attn/TrainDataset/attention_{args.conf}_{args.max_det}/')

    image_root_val = Path(f'{args.val_path}/images/')
    gt_root_val = Path(f'{args.val_path}/masks/')
    attn_map_root_val_1 = Path(
        f'./dataset_attn/ValDataset/attention_{args.conf}_{args.max_det}/')

    train_loader = get_attn_loader(
        image_root=image_root,
        gt_root=gt_root,
        attn_map_root_1=attn_map_root_1,
        attn_map_root_2=None,
    ...
    val_loader = get_attn_loader(
        ...
        gt_root=gt_root_val,
        attn_map_root_1=attn_map_root_val_1,
        attn_map_root_2=None,
        batchsize=args.batchsize,
        trainsize=args.trainsize,
        phase='val',
