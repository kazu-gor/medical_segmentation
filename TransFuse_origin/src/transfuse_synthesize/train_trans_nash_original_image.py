...
def nash_analytic_v1(losses, shared_parameters, task_specific_parameters):
    ...
    alpha_1 = torch.sqrt(
        torch.sqrt(g2_g2) / (g1_g1 * torch.sqrt(g2_g2) + g1_g2 * torch.sqrt(g1_g1))
    )
    alpha_2 = torch.sqrt(
        torch.sqrt(g1_g1) / (g2_g1 * torch.sqrt(g2_g2) + g2_g2 * torch.sqrt(g1_g1))
    )
...
def train(dataloaders_dict, models, optimizer, criterion, epoch, best_loss, best2_loss):
    ...
    for phase in ["train", "val"]:
        ...
        size_rates = [1]
        loss_record2, loss_record3, loss_record4, d_loss_record = (
            AvgMeter(),
            AvgMeter(),
            AvgMeter(),
            AvgMeter(),
        ...
        for i, pack in enumerate(dataloaders_dict[phase], start=1):
            for rate in size_rates:
                ...
                _image = (
                    _image.add(torch.FloatTensor(IMAGENET_MEAN).view(3, 1, 1))
                    .detach()
                    .cuda()
                )
                ...
                with torch.set_grad_enabled(phase == "train"):
                    # ---- forward ----
                    lateral_map_4, lateral_map_3, lateral_map_2, dis_in = models[
                        "Transfuse"
                    ](images)
                    ...
                    dis_in = transform_norm(IMAGENET_MEAN, IMAGENET_STD)(dis_in)
            ...
            if (i % 20 == 0 or i == total_step) and phase == "train":
                print(
                    ...
                    "[lateral-2: {:.4f}, lateral-3: {:0.4f}, lateral-4: {:0.4f}, d_loss: {:0.4f}".format(
                        datetime.now(),
                        epoch,
                        opt.epoch,
                        i,
...
if __name__ == "__main__":
    ...
    shared_parameters = [
        ...
        if "final_x" not in n and "final_1" not in n and "cls" not in n
    ]
    task_specific_parameters = [
        p
        for n, p in model1.named_parameters()
        if "final_x" in n or "final_1" in n or "cls" in n
    ]
