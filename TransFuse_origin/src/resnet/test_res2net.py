...
parser.add_argument(
    "--pth_path2", type=str, default="./snapshots/train_res2net/Discriminator-59.pth"
)
...
for i in range(test_loader1.size):
    ...
    with torch.no_grad():
        ...
        _image = (
            _image.add(torch.FloatTensor(IMAGENET_MEAN).view(3, 1, 1)).detach().numpy()
        )
        _image = _image.squeeze()
        _image = np.transpose(_image, (1, 2, 0))
        _image = (_image - _image.min()) / (_image.max() - _image.min() + 1e-8)
        # imageio.imsave(save_path + '_image_' + name, img_as_ubyte(_image))
