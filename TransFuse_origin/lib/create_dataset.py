    ...
    def __call__(self, mask_list, height=352, width=352):
        ...
        for mask in mask_list:
            ...
            for i in range(len(contours)):
                ...
                contour_arr = np.concatenate((contour_arr, coordinate_list), axis=0)
...
class CreateDataset:
    ...
    def get_no_parameter_dataset(self):
        ...
        for image_path, mask_path in tqdm(zip(self.image_list, self.mask_list)):
            ...
            # 3. Save the image
            cv2.imwrite(
                str(self.output_path / "images" / f"left_{image_path.stem}.png"),
                image_left,
            )
            cv2.imwrite(
                str(self.output_path / "images" / f"right_{image_path.stem}.png"),
                image_right,
            )
            cv2.imwrite(
                str(self.output_path / "masks" / f"left_{mask_path.stem}.png"),
                mask_left,
            )
            cv2.imwrite(
                str(self.output_path / "masks" / f"right_{mask_path.stem}.png"),
                mask_right,
            )
    ...
    def get_divide_data(self):
        ...
        train_sekkai_image_list = self._get_image_list(
            "../../../dataset/sekkai_TrainDataset/images/"
        )
        test_sekkai_image_list = self._get_image_list(
            "../../../dataset/sekkai_TestDataset/images/"
        )
        val_sekkai_image_list = self._get_image_list(
            "../../../dataset/sekkai_ValDataset/images/"
        )
        ...
        for image_path, mask_path in tqdm(zip(no_para_image_list, no_para_mask_list)):
            ...
            cv2.imwrite(
                str(self.output_path / f"all/images/{mode}/{image_path}.png"), image
            )
            cv2.imwrite(
                str(self.output_path / f"all/masks/{mode}/{mask_path}.png"), mask
            )
            ...
            cv2.imwrite(
                str(self.output_path / f"sekkai/images/{mode}/{image_path}.png"), image
            )
            cv2.imwrite(
                str(self.output_path / f"sekkai/masks/{mode}/{mask_path}.png"), mask
            )
    ...
    def _plot_box(self, image_path, mask_path, anno):
        ...
        for a in anno.astype("int64"):
            cv2.rectangle(image, (a[0], a[1]), (a[2], a[3]), (0, 255, 0), 2)
            cv2.imwrite(f"./output/plot/images/{image_path.stem}.png", image)
            cv2.rectangle(mask, (a[0], a[1]), (a[2], a[3]), (0, 255, 0), 2)
    ...
    def _make_label(self, image_list, mask_list, mode, dir_name):
        for image_path, mask_path in tqdm(zip(image_list, mask_list)):
            ...
            with open(
                f"{self.output_path}/{mode}/labels/{dir_name}/{image_path.stem}.txt",
                "w",
            ) as f:
                for a in anno:
                    f.write(
